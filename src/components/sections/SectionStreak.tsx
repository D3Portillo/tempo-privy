...
export function SectionStreak() {
  ...
  const [isFreezeSetupOpen, setIsFreezeSetupOpen] = useState(false)
              ...
              <div className="mt-4 rounded-3xl border border-white/20 bg-white/5 p-6">
                <div className="flex items-center justify-between">
                  ...
                  {freezeCount < 2 ? (
                    <button
                      type="button"
                      onClick={() => setIsFreezeSetupOpen(true)}
...
type StreakSetupModalProps = {
  isOpen: boolean
  onClose: () => void
}

function StreakSetupModal({ isOpen, onClose }: StreakSetupModalProps) {
  return (
    <BaseModal
      ariaLabel="Streak setup"
      title="Streak Setup"
      isOpen={isOpen}
      onClose={onClose}
    >
      <p className="mt-4 text-sm text-white/80">
        Protect your streak with a small fee — it goes straight into your
        Partner Vault for gifts or any shared plans.
      </p>

      <button
        type="button"
        onClick={onClose}
        className="mt-6 w-full rounded-xl bg-wb-violet p-3 text-sm font-semibold text-white"
      >
        Confirm Setup ($5)
      </button>
    </BaseModal>
